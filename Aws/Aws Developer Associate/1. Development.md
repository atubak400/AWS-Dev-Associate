...
- Pre-signed URLs allow temporary and secure access to private S3 objects without exposing the bucket or object directly.
...
- To enable detailed monitoring on an existing EC2 instance, use the aws ec2 monitor-instances command with the --instance-ids flag followed by the instance ID. This ensures that additional metrics are captured at a 1-minute interval for better analysis.
...
- For example: aws ec2 monitor-instances --instance-ids i-1234567890abcdef0 or aws ec2 monitor-instances --instance-ids i-1234567890abcdef0 i-1234353890advfef0
...
### 182. Using Lambda Layers for Shared Dependencies
...
### 184. mproving Lambda Function Performance
...
### 187. Handling Failed Asynchronous Invocations
- DLQs are often associated with SQS, but they also work with other AWS services like Lambda. A DLQ can capture failed asynchronous invocations. If Lambda cannot process an event after retries, the event is sent to the DLQ (SQS or SNS) for later analysis or reprocessing.
- Use Lambda Destinations to route the results of asynchronous invocations (success or failure) to services like SQS, SNS, or EventBridge for further processing. Combine this with a Dead-Letter Queue (DLQ) to capture and analyze failed invocations, ensuring reliable error tracking and enabling re-processing if needed. This setup enhances resilience and debugging capabilities.
...
### 122. Granting CodeCommit Access for CodeDeploy and EC2
...
### 123. Elastic Network Interfaces (ENIs)
...
### 124. Execution Role and ENI Permissions
